...
let items: any[] = [];
let ingredients: any[] = [];
let ticket: { item: any; qty: number }[] = [];
let selectedIngredient: any = null;
...
function demoGet(path: string) {
  ...
  if (base === "items") return DEMO_ITEMS;
  if (base === "ingredients") return DEMO_INGREDIENTS;
  if (base === "summary") {
  ...
  if (base === "ledger") return demoLedger.slice(0, 15);
  return [];
}

function demoPost(path: string, body: any) {
  if (path === "item") {
...
document.querySelectorAll(".tab").forEach((tab) => {
  tab.addEventListener("click", () => {
    ...
    if (tabEl.dataset.tab === "reports") refreshReports();
  });
});
...
$("createItemForm")?.addEventListener("submit", async (e) => {
  e.preventDefault();
  const nameInput = $("newItemName") as HTMLInputElement;
  const priceInput = $("newItemPrice") as HTMLInputElement;
  const catInput = $("newItemCategory") as HTMLInputElement;
  const btn = $("saveItemBtn") as HTMLButtonElement;
...
$("createIngredientForm")?.addEventListener("submit", async (e) => {
  e.preventDefault();
  const nameInput = $("newIngName") as HTMLInputElement;
  const unitInput = $("newIngUnit") as HTMLInputElement;
  const stockInput = $("newIngStock") as HTMLInputElement;
  const reorderInput = $("newIngReorder") as HTMLInputElement;
  const btn = $("saveIngBtn") as HTMLButtonElement;
...
function wireSearch(inputId: string, suggestId: string, source: () => any[], onPick: (id: string) => void) {
  ...
  document.addEventListener("click", (e) => {
    if (!(e.target as HTMLElement).closest(`#${inputId}, #${suggestId}`)) list.classList.add("hidden");
  });
}
...
wireSearch(
  "itemSearch",
  "itemSuggest",
  () => items.map((i) => ({ id: i._id, label: i.name, sub: money(i.price) })),
  (id) => {
    const it = items.find((i) => i._id === id);
    if (it) addToTicket(it);
  }
);
...
function renderTicket() {
  ...
  if (!ticket.length) {
    ...
    ticket.forEach((line, idx) => {
      ...
      row.querySelector('[data-act="dec"]')!.addEventListener("click", () => {
        line.qty--;
        if (line.qty <= 0) ticket.splice(idx, 1);
        renderTicket();
      });
      box.appendChild(row);
    });
  ...
  ($("completeSaleBtn") as HTMLButtonElement).disabled = ticket.length === 0;
}
...
$("completeSaleBtn")!.addEventListener("click", async () => {
  const btn = $("completeSaleBtn") as HTMLButtonElement;
  btn.disabled = true;
  try {
...
wireSearch(
  "ingredientSearch",
  "ingredientSuggest",
  () => ingredients.map((i) => ({ id: i._id, label: i.name, sub: `${i.stock} ${i.unit}` })),
...
function selectIngredient(id: string) {
  ...
  ($("logPurchaseBtn") as HTMLButtonElement).disabled = false;
  updatePurchaseTotal();
}

function updatePurchaseTotal() {
  const qty = parseFloat(($("purchaseQty") as HTMLInputElement).value) || 0;
  const cost = parseFloat(($("purchaseCost") as HTMLInputElement).value) || 0;
...
$("purchaseForm")!.addEventListener("submit", async (e) => {
  e.preventDefault();
  if (!selectedIngredient) return;
  const qty = parseFloat(($("purchaseQty") as HTMLInputElement).value);
  const unitCost = parseFloat(($("purchaseCost") as HTMLInputElement).value);
  const supplier = ($("purchaseSupplier") as HTMLInputElement).value;
  try {
    ...
    ($("logPurchaseBtn") as HTMLButtonElement).disabled = true;
    ingredients = await apiGet("ingredients");
    renderStockGrid();
    await refreshReports();
  } catch (e) {
...
async function refreshReports() {
  try {
    ...
    lowBox.innerHTML = summary.lowStock.length
      ? summary.lowStock
        .map((s: any) => `<div class="row crit"><span>${s.name}</span><span>${s.stock} ${s.unit} left</span></div>`)
        .join("")
  ...
  btn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast(msg || `No ${type} available to export.`, true);
        return;
      }

      const blob = await res.blob();
      if (!blob.size) {
        toast(`No ${type} available to export.`, true);
        return;
      }
      ...
      toast("Could not export CSV.", true);
    }
  });
});
...
window.addEventListener("load", () => {
  ...
  (window as any).google?.accounts.id.renderButton($("googleBtn"), { theme: "filled_black", size: "medium" });
